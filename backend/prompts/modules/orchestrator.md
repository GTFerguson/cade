...
- `mode`: worker mode — `"code"` (full access), `"plan"` (read-only), `"research"` (PROVEN research pipeline), `"review"` (code review), `"orchestrator"` (spawn further sub-agents). Match this to the subtask type.
